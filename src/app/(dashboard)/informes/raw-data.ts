...
export async function fetchRawData(supabase: SupabaseClient): Promise<RawData> {
  const [{ data: companies }, { data: contacts }, { data: opportunities }, { data: expenses }, { data: invoiceItems }] =
    await Promise.all([
      ...
      supabase.from("invoice_items").select("quantity, unit_price, invoices!inner(issue_date, tax_rate, status)"),
    ]);

  // Se suma por línea directamente sobre un mapa indexado por el objeto de
  // factura (identity), para no depender de un identificador compuesto.
  const perInvoice = new Map<object, { issue_date: string; status: string; total: number }>();
  for (const row of invoiceItems ?? []) {
    ...
    const acc = perInvoice.get(invoice) ?? { issue_date: invoice.issue_date, status: invoice.status, total: 0 };
    acc.total += lineTotal;
    perInvoice.set(invoice, acc);
  }
