...
export type SeriesRow = { id: string; metric: MetricKey; color: string };

export type ReportBuilderInitial = {
  id?: string;
  name: string;
  chartType: ChartType;
  series: { metric: MetricKey; color: string }[];
  dateFrom: string;
  dateTo: string;
...
export function ReportBuilderForm({
  ...
  const [rows, setRows] = useState<SeriesRow[]>(
    initial && initial.series.length > 0
      ? initial.series.map((s) => ({ id: crypto.randomUUID(), metric: s.metric, color: s.color }))
      : [{ id: crypto.randomUUID(), metric: METRICS[0].key, color: colorAt(0) }],
  );
  const [dateFrom, setDateFrom] = useState(initial?.dateFrom ?? "");
  const [dateTo, setDateTo] = useState(initial?.dateTo ?? "");
  const [comparePrevious, setComparePrevious] = useState(initial?.comparePrevious ?? false);
